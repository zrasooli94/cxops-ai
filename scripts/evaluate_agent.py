...
CASES = [
    ...
    },
    {
        ...
        "expected_tool": ("zendesk.update_ticket"),
        "expected_auto_execute": False,
    },
    {
        ...
        "expected_retrieval": False,
        "expected_tool": ("zendesk.add_internal_note"),
...
async def main() -> None:
    ...
    async with AsyncSessionLocal() as db:
        for index, case in enumerate(
            ...
            print(f"[{index}/{len(CASES)}] Evaluating: {case['name']}")
            ...
            print(
                f"Tools: {result['actual_tools']} (expected {result['expected_tool']})"
            )
    ...
    action_passed = sum(1 for result in results if result["action_pass"])

    retrieval_passed = sum(1 for result in results if result["retrieval_pass"])

    tool_passed = sum(1 for result in results if result["tool_pass"])

    auto_execute_passed = sum(1 for result in results if result["auto_execute_pass"])
    ...
    report = {
        ...
        "action_accuracy": (action_passed / total if total else 0),
        "retrieval_accuracy": (retrieval_passed / total if total else 0),
        "tool_accuracy": (tool_passed / total if total else 0),
        "auto_execute_safety_accuracy": (auto_execute_passed / total if total else 0),
        ...
        "fastest_case": (
            {
                ...
                "latency_ms": (fastest_case["latency_ms"]),
            }
            if fastest_case
            else None
        ),
        "slowest_case": (
            {
                ...
                "latency_ms": (slowest_case["latency_ms"]),
            }
            if slowest_case
            else None
        ),
    ...
    output = Path("evals/latest_agent_report.json")
    ...
    print(f"Overall pass rate: {percentage(passed, total):.1f}%")

    print(f"Action accuracy: {percentage(action_passed, total):.1f}%")

    print(f"RAG routing accuracy: {percentage(retrieval_passed, total):.1f}%")

    print(f"Tool selection accuracy: {percentage(tool_passed, total):.1f}%")

    print(f"Auto-execution safety: {percentage(auto_execute_passed, total):.1f}%")

    print(f"Average latency: {average_latency_ms:.2f}ms")

    if fastest_case:
        print(f"Fastest case: {fastest_case['name']} ({fastest_case['latency_ms']}ms)")

    if slowest_case:
        print(f"Slowest case: {slowest_case['name']} ({slowest_case['latency_ms']}ms)")
    ...
    if failed_cases:
        ...
        for failure in failed_cases:
            print(f"\n{failure['name']} (ticket {failure['ticket_id']})")
